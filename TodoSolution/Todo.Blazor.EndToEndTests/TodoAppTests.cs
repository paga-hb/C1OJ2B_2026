...
public class TodoAppTests : IClassFixture<PlaywrightFixture>
{
    ...
    public async Task HomePage_NavigateTo_NewTodoItem_ThenEditAndSave_ShouldContainNewTodoItemInHomePageTodoItemList()
    {
        ...
        //await page.GetByLabel("Name").ClickAsync(); // <--- alternatively, this also works

        // Fill in the text "Yakety Yak"        
        await page.GetByRole(AriaRole.Textbox, new() { Name = "Name" }).FillAsync("Yakety Yak");
        //await page.GetByLabel("Name").FillAsync("Yakety Yak"); // <--- alternatively, this also works
        ...
        await page.GetByRole(AriaRole.Textbox, new() { Name = "Notes" }).ClickAsync();
        //await page.GetByLabel("Notes").ClickAsync(); // <--- alternatively, this also works

        // Fill in the text "Take out the papers and the trash"
        await page.GetByRole(AriaRole.Textbox, new() { Name = "Notes" }).FillAsync("Take out the papers and the trash");
        //await page.GetByLabel("Notes").FillAsync("Take out the papers and the trash"); // <--- alternatively, this also works

        // Assert the input field contains the expected text // <--- Added this assertion
        (await page.GetByLabel("Notes").InputValueAsync()).Should().Be("Take out the papers and the trash");
        
        // Check the Done checkbox
        await page.GetByRole(AriaRole.Checkbox, new() { Name = "Done" }).CheckAsync();
        //await page.GetByLabel("Done").CheckAsync(); // <--- alternatively, this also works
        ...
        //await page.GetByRole(AriaRole.Button, new() { Name = "Save" }).ClickAsync(); // <--- alternatively, this also works

        // Wait 200ms (we want to ensure the TodoItem has been added to the database before visiting the Home Page again)
